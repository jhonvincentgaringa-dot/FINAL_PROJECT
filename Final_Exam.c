...
// line 36-51 Function declarations
...
int main() {
    // line 54-63 local variables and pointers are allocated on the Call Stack.
    ...
    do {
        ...
        switch (choice) {
        	// line 104-133 for the student registration
        case 1:
        ...
        case 5:
        	// line 170-173 for student sorted by id
            bubbleSortStudents(head);
            printf(GREEN BOLD "Students sorted successfully by ID!\n" RESET);
            break;

        case 6:
        	// line 176-179 for student sorted by name
...
// line 228-252 for stack functions and undo function
void initStack(struct Stack *s) {
    s->top = NULL;
}
...
// line 252-262 Function to check if a string contains only numeric digits
int isNumeric(const char *str) {
    if (*str == '\0') return 0;
    while (*str) {
...
// line 265-271 Function to check if a string contains at least one numeric digits
int hasNumber(const char *str) {
    while (*str) {
...
// line 274-286 for the student registration 
struct Student *createStudent(const char *id, const char *name, const char *course, const char *major) {
...
// line 293-305 for adding student
void addStudent(struct Student **head, const char *id, const char *name, const char *course, const char *major) {
...
// Line 303-316 visit every node in the list for displaying the student information
void displayStudents(struct Student *head) {
    if (head == NULL) {
        printf(RED "No students registered yet.\n" RESET);
        return;
...
// line 320-329 iterates through the list until the target ID is found for finding student id
struct Student *findStudent(struct Student *head, const char *id) {
    struct Student *current = head;
    while (current != NULL) {
...
//line 333-366 for editing existing student data
void editStudent(struct Student *head, const char *id) {
...
// line 369-397 for deleting student data (and pushing to Stack for Undo feature)
void deleteStudent(struct Student **head, const char *id, struct Stack *stack) {
    struct Student *current = *head;
...
// line 400-417 for swapping student data 
void swapStudentData(struct Student *a, struct Student *b) {
...
// line 423-444 for sorting by id
void bubbleSortStudents(struct Student *head) {
    int swapped;
    struct Student *ptr1;
...
// line 451-473 for sorting by name
void selectionSortByName(struct Student *head) {
