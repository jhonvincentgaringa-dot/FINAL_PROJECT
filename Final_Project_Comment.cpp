...
// Lines 8-14 define the structure and the pointer to the next node
struct Student {
...
int main() {
    ...
    // Lines 29-34 Local variables stored on the Stack memory
    ...
    do {
        ...
        switch (choice) {
        	...
        	// lines 55-84 for the student registration
            case 1:
            ...
            case 2:
            	// lines 88-89 for displsying the student registered 
                displayStudents(head);
                break;

            case 3:
                {
                	// lines 94-109 for finding student details
            ...
            case 4:
            	// line 113-117 for deleting student registered
...
// Lines 187-202 visit every node in the list
void displayStudents(struct Student *head) {
    if (head == NULL) {
...
// Lines 206-215 iterate through the list until the target ID is found
struct Student *findStudent(struct Student *head, const char *id) {
    struct Student *current = head;
    while (current != NULL) {
